...
@MainActor
final class MacViewModel: ObservableObject {
    ...
    private let key = "starflower.mac.location.v1"
    ...
    // ── 이하 iOS StargazingViewModel에서 그대로 복사 ──────────
    private func buildData(_ loc: SavedLocation, _ wx: WeatherResponse, _ air: AirResponse?) -> StargazingData {
        ...
        for i in 0..<3 {
            ...
            let exposure = MoonCalculator.nightExposure(start: nightStart(eStr), end: nightEnd(eStr), lat: lat, lng: lng)
        ...
        return StargazingData(
            location: loc, score: forecast.first?.score ?? 0,
    ...
    private func nightStart(_ e: String) -> Date { var c = ymd(e)!; c.hour = 19; return cal.date(from: c) ?? Date() }
    private func nightMidnight(_ e: String) -> Date { var c = ymd(e)!; c.hour = 24; return cal.date(from: c) ?? Date() }
    private func nightEnd(_ e: String) -> Date { var c = ymd(e)!; c.day! += 1; c.hour = 5; return cal.date(from: c) ?? Date() }
    private func avg(_ v: [Double]) -> Double { let f = v.filter { $0.isFinite }; return f.isEmpty ? 0 : f.reduce(0,+)/Double(f.count) }
    ...
    private func classifySky(_ code: Int, _ cloud: Double) -> SkyCondition {
        ...
        if cloud < 15 { return .clear }; if cloud < 50 { return .partly }
        if cloud < 85 { return .cloudy }; return .overcast
    }
